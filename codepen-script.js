...
class Planet {
  constructor (name, numOfRows, numOfColumns) {
    // check parameters
    if (typeof numOfRows === 'undefined' || typeof numOfColumns === 'undefined') {
      console.log(`error: number or Rows or Columns was undefined. Please declare it`)
...
class Rover {
  ...
  set direction(newDirection) {
    if (['N', 'S', 'E', 'W'].includes(newDirection) && typeof newDirection === 'string') {
  ...
  _parseInput(inputArr) {
    ...
    if (options.blockBadInputs) {
      if ( flattenedNewInputArr.every( e => { return ['f', 'b', 'l', 'r'].includes(e)}) ) {
        return flattenedNewInputArr
      } else {
        console.log(`one or more inputs for .move() are invalid. Please only pass strings or objects with the values: ${['f', 'b', 'l', 'r']}`)
      }
      ...
      if (removedInputValues.length > 0) {
        console.log(`the following inputs were invalid and ignored: ${removedInputValues}`);
      }
  ...
  _turnLeft() {
    ...
    console.log(`The ${this.name} rover turned left and is now facing: ${this._direction}`)
  }
  _turnRight() {
    switch (this._direction) {
      case 'N':
    ...
    console.log(`The ${this.name} rover turned right and is now facing: ${this._direction}`)
  }
  _moveFoward() {
    switch(this._direction) {
      case 'N':
        if ( this.position.r-1 >= 0 ) {
          ...
          console.log(`The ${this.name} rover moved northward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
          console.log(`The ${this.name} rover didn't move northward. position: r = ${this.position.r}, c = ${this.position.c} ${options.outOfBoundsMsg}`)
        }
        break;
      case 'W':
        if (this.position.c-1 >= 0) {
          ...
          const newPosition = {r: this.position.r, c: this.position.c};
          this.travelLog.push(newPosition)
          console.log(`The ${this.name} rover moved westward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
          console.log(`The ${this.name} rover didn't move westward. position: r = ${this.position.r}, c = ${this.position.c} ${options.outOfBoundsMsg}`)
        }
        break;
      case 'S':
        if ( this.position.r+1 <= this.planet.board.length ) {
          ...
          const newPosition = {r: this.position.r, c: this.position.c};
          this.travelLog.push(newPosition)
          console.log(`The ${this.name} rover moved southward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
      ...
      case 'E':
        if ( this.position.c+1 <= this.planet.board[0].length ) {
          ...
          const newPosition = {r: this.position.r, c: this.position.c};
          this.travelLog.push(newPosition)
          console.log(`The ${this.name} rover moved eastward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
          console.log(`The ${this.name} rover didn't move eastward. position: r = ${this.position.r}, c = ${this.position.c} ${options.outOfBoundsMsg}`)
        }
        break;
    }
  }
    ...
    switch(this._direction) {
      case 'N':
        if ( this.position.r+1 <= this.planet.board.length ) {
          ...
          const newPosition = {r: this.position.r, c: this.position.c};
          this.travelLog.push(newPosition)
          console.log(`The ${this.name} rover moved, in reverse, southward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
      ...
      case 'W':
        if ( this.position.c+1 <= this.planet.board[0].length ) {
          ...
          const newPosition = {r: this.position.r, c: this.position.c};
          this.travelLog.push(newPosition)
          console.log(`The ${this.name} rover moved, in reverse, eastward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
          console.log(`The ${this.name} rover couldn't go further. position: r = ${this.position.r}, c = ${this.position.c} ${options.outOfBoundsMsg}`)
        }
        break;
      case 'S':
        if ( this.position.r-1 >= 0 ) {
          ...
          const newPosition = {r: this.position.r, c: this.position.c};
          this.travelLog.push(newPosition)
          console.log(`The ${this.name} rover moved, in reverse, northward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
          console.log(`The ${this.name} rover couldn't go further. position: r = ${this.position.r}, c = ${this.position.c} ${options.outOfBoundsMsg}`)
        }
        break;
      case 'E':
        if (this.position.c-1 >= 0) {
          ...
          const newPosition = {r: this.position.r, c: this.position.c};
          this.travelLog.push(newPosition)
          console.log(`The ${this.name} rover moved, in reverse, westward. position: r = ${this.position.r}, c = ${this.position.c}`)
        } else {
          console.log(`The ${this.name} rover couldn't go further. position: r = ${this.position.r}, c = ${this.position.c} ${options.outOfBoundsMsg}`)
        }
        break;
